...
- [ ] **Phase 3.2** *(spike + capture session complete 2026-05-21 — implementation pending)* — Replace JSESSIONID-cookie auth with mobile-app Bearer token (`monitoring-eu.epcube.com/api/device/*`). Same host, unified write API (one `switchMode` endpoint vs our three — but requires full payload every call), programmatically-solvable captcha (no human in the loop), and a one-time email+password in config-flow replaces the hourly cookie paste. Live-cloud capture session validated end-to-end write path against our real device (read + write + read-back, no-op). Confirmed corrections to the spike doc: `Authorization: Bearer <token>` requires the prefix, `Accept-Language: en-GB` should be used, `onlySave` is persistent state (must always send `"0"`), `weatherWatch` is a forecast-driven pre-charge feature that fights Predbat (must always send `"0"`). Full assessment + capture findings at [docs/PHASE_3_2.md](docs/PHASE_3_2.md). One known risk to plan for: opening the official mobile app silently revokes write privileges on the HA token (mitigations: post-write read-back, auto re-auth, Predbat back-off). Worth landing before Phase 4 HACS so new users don't hit the JSESSIONID UX.
