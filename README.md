...
- [ ] **Phase 3.2** *(candidate)* — Investigate Bobsilvio's mobile-app API surface (`monitoring-eu.epcube.com/api/device/*` with Bearer token via his external `epcube-token` tool). Same backend, different path prefix, no slider captcha, long-lived tokens — would eliminate the JSESSIONID-paste-every-hour UX. Real rewrite of api.py auth + endpoints.
