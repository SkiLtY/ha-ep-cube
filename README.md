...
- [ ] **Phase 4** — HACS distribution, light test scaffolding, GitHub Actions. *Session 22 (2026-05-25) shipped distribution scaffolding* — `hacs.json` at repo root with `zip_release` config, [`.github/workflows/validate.yml`](.github/workflows/validate.yml) (hassfest + HACS action + weekly cron) and [`.github/workflows/release.yml`](.github/workflows/release.yml) (tag-fired manifest-verify + zip + GitHub release with auto notes), `manifest.json` key-sort, brand asset stash in [`docs/brand/`](docs/brand/) with README mapping each source to its eventual home-assistant/brands target. *Session 24 (2026-05-27) executed the two-repo split + public flip* — repo is now public, validate workflow runs Hassfest ✅ + HACS 7/8 ✅ on every push (only `brands` check outstanding, deferred until home-assistant/brands PR). Remaining: brands PR (clears last HACS check + qualifies for HACS default-list submission); relocate this repo's example `ha_config/` helpers (Riemann load_today + input_numbers Predbat writes to) into `examples/ha_config/` so HACS users can copy them; tests scaffolding (third leg of Phase 4); first semver release (`v0.1.0`) firing `release.yml`.
