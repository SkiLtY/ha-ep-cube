...
- [ ] **Phase 3.4** — Feature-parity catch-up vs [Bobsilvio/epcube](https://github.com/Bobsilvio/epcube) before HACS publication. Ordered backlog: (a) multi-region (US/JP) endpoints in config flow; (b) `EntityCategory.CONFIG`/`DIAGNOSTIC` tagging on existing sensors; (c) mode `select` entity; (d) "allow charging from grid" `switch`; (e) SoC reserve `number` entities; (f) `set_tou_schedule` service (DST + workday/weekend + per-slot prices); (g) `set_operating_mode` service; (h) i18n / translations file; (i) daily/monthly/annual energy sensors.
