...
| 4.1 | ✅ | **TOU schedule editor** — shipped in [`v0.6.0`](https://github.com/SkiLtY/ha-ep-cube/releases/tag/v0.6.0); UX polish + card hydration follow-on in [`v0.6.1`](https://github.com/SkiLtY/ha-ep-cube/releases/tag/v0.6.1) + [`v0.6.2`](https://github.com/SkiLtY/ha-ep-cube/releases/tag/v0.6.2). `ep_cube.set_tou_schedule` service + bundled Lovelace editor card with workday/weekend tabs, inline validation, shim coexistence, smarter slot defaults, copy-other-profile helper, and hydration from cube state (card shows real cube schedule on load, after save, and via *Reload from cube*). |
