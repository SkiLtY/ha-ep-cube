...
- [x] **Phase 1** — Mock cloud + HA integration skeleton + 9 sensors + DeviceInfo *(verified, 2026-05-03)*
...
- [x] **Phase 3.3** — Animated power-flow Lovelace dashboard YAML (built on the [`power-flow-card-plus`](https://github.com/flixlix/power-flow-card-plus) HACS card) shipping in [`dashboards/ep_cube.yaml`](dashboards/ep_cube.yaml) + README install steps, so users get an EP-Cube-app-style live view without needing to look at Predbat. *Verified live 2026-05-22.* Layout: power flow at top, battery status, operating-mode picker, then conditional cards keyed off `select.ep_cube_operating_mode` (Self Consumption / Backup / TOU). TOU card has a slot reserved for the Phase 4.1 schedule editor. Bring-up surfaced five card-config gotchas — see commits [`4fafef7`](https://github.com/SkiLtY/ha-ep-cube/commit/4fafef7), [`0cdb73d`](https://github.com/SkiLtY/ha-ep-cube/commit/0cdb73d), [`01c22a7`](https://github.com/SkiLtY/ha-ep-cube/commit/01c22a7), [`ae2b2d0`](https://github.com/SkiLtY/ha-ep-cube/commit/ae2b2d0) and the inline YAML comments for the wire-level details.
- [x] **Phase 3.4** — Feature-parity catch-up vs [Bobsilvio/epcube](https://github.com/Bobsilvio/epcube) before HACS publication. **Closed 2026-05-24 session 21** with (i) shipped ([`b1ed9a9`](https://github.com/SkiLtY/ha-ep-cube/commit/b1ed9a9)): 5 user-facing daily kWh sensors (`solar_today`, `grid_today`, `backup_today`, `nonbackup_today`, `self_consumption_pct`) + 3 DIAGNOSTIC (`solar_dc_today`, `solar_ac_today`, `winter_protect`) + 8 `utility_meter` rollups (monthly + yearly × 4 user-facing) — all data piggybacks on the existing `homeDeviceInfo` poll, zero new cloud calls. `grid_today` is direction-ambiguous magnitude — Energy Dashboard section in this README points users at the Riemann `sensor.ep_cube_{import,export}_today` for clean signed direction. `RestoreSensor` ([`6055d7c`](https://github.com/SkiLtY/ha-ep-cube/commit/6055d7c)) keeps sensor values stable across HA restarts (was a real UX paper-cut). Earlier history — manual-control surface landed 2026-05-22 ([`d982aec`](https://github.com/SkiLtY/ha-ep-cube/commit/d982aec), [`0abc91c`](https://github.com/SkiLtY/ha-ep-cube/commit/0abc91c), [`4b2a856`](https://github.com/SkiLtY/ha-ep-cube/commit/4b2a856), [`c58fe50`](https://github.com/SkiLtY/ha-ep-cube/commit/c58fe50)): (c) `select.ep_cube_operating_mode`, (d) `switch.ep_cube_allow_grid_charge`, (e) `number.ep_cube_self_consumption_reserve` + `number.ep_cube_backup_reserve`, plus bonus `switch.ep_cube_daylight_saving_time`. All mode-aware via dynamic `available` + write pre-check (cube silently ignores cross-mode writes — see [TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md)). Multi-region + entity-category tagging shipped 2026-05-22: (a) [`7d7f608`](https://github.com/SkiLtY/ha-ep-cube/commit/7d7f608) — EU/US/JP/Other dropdown in the config flow (US uses a different host **and** path prefix `/app-api`, so `api_prefix` is now threaded through `EPCubeClient` + `captcha.login`; config-entry `VERSION=5` with `async_migrate_entry` to infer region from the stored base_url for existing entries); (b) [`ab536ac`](https://github.com/SkiLtY/ha-ep-cube/commit/ab536ac) — `EntityCategory.DIAGNOSTIC` on the dupe sensors (`battery_capacity_kwh` / `operating_mode` / `reserve_soc`) + `EntityCategory.CONFIG` on all writables (select / switches / numbers). Remaining backlog: (i) daily/monthly/annual energy sensors (gated on cloud-data availability). *(f)* `set_tou_schedule` rescoped to Phase 4.1 session 19 — bundled with a Lovelace editor card so service + UX land together (HACS-frontend distribution). *(g)* `set_operating_mode` dropped session 19 ([`f508c6b`](https://github.com/SkiLtY/ha-ep-cube/commit/f508c6b)) — superseded by `select.ep_cube_operating_mode` + reserve `number` entities. *(h)* `de`/`it`/`nl` translations shipped session 19 ([`469091d`](https://github.com/SkiLtY/ha-ep-cube/commit/469091d)); `ja` deferred.
- [x] **Phase 3.5** — Bobsilvio-parity metrics expansion. **Closed 2026-05-25 session 22** ([`e0eb77f`](https://github.com/SkiLtY/ha-ep-cube/commit/e0eb77f)). Five new read-only sensors: `earning_yesterday` (£, MONETARY), `grid_outage_count` (DIAGNOSTIC), `off_grid_seconds` (DIAGNOSTIC, DURATION), `battery_charge_today` + `battery_discharge_today` (kWh, client-side delta-tracker on `batteryCurrentElectricity` with 0.05 kWh jitter threshold — cube doesn't expose signed battery flow). Plus 4 `utility_meter` monthly/yearly rollups on the battery sensors + de/en/it/nl translations + mock parity. Lifetime battery totals deferred to Phase 4.2 (needs RestoreSensor + state-seeding).
- [ ] **Phase 4** — HACS distribution, light test scaffolding, GitHub Actions. *Session 22 (2026-05-25) shipped distribution scaffolding* — `hacs.json` at repo root with `zip_release` config, [`.github/workflows/validate.yml`](.github/workflows/validate.yml) (hassfest + HACS action + weekly cron) and [`.github/workflows/release.yml`](.github/workflows/release.yml) (tag-fired manifest-verify + zip + GitHub release with auto notes), `manifest.json` key-sort, brand asset stash in [`docs/brand/`](docs/brand/). *Session 24 (2026-05-27) executed the two-repo split + public flip* — repo is now public, validate workflow runs Hassfest ✅ + HACS 7/8 ✅ on every push. *Session 25 (2026-05-27) closed brand assets* — submitted brands-repo PR [#10389](https://github.com/home-assistant/brands/pull/10389), bot auto-closed it per the [Feb 2026 brands-proxy-api change](https://developers.home-assistant.io/blog/2026/02/24/brands-proxy-api) (custom integrations now self-host brand PNGs). Switched approach: 8 SVG-sourced PNGs now ship inside [`custom_components/ep_cube/brand/`](custom_components/ep_cube/brand/) and HA 2026.3+ serves them at `/api/brands/integration/ep_cube/<image>`, taking priority over the brands CDN. The `brands` HACS check is therefore moot for our case. Remaining: relocate this repo's example `ha_config/` helpers (Riemann load_today + input_numbers Predbat writes to) into `examples/ha_config/` so HACS users can copy them; tests scaffolding (third leg of Phase 4); first semver release (`v0.1.0`) firing `release.yml`.
- [ ] **Phase 4.1** — TOU schedule editor (was Phase 3.4 (f)): `set_tou_schedule` service + Lovelace editor card that slots into the dashboard's reserved TOU panel. Post-HACS so real EU/DE/IT/NL/AU/US users can shape the UX and the card distributes via HACS-frontend.
- [ ] **Phase 4.2** — Cloud-stats endpoint expansion. Capture `GET /device/queryDataElectricityV2?devId=X&queryDateStr=Y&scopeType=Z` (4 scopeTypes: daily/monthly/annual/total) via mitmproxy on phone, wire as separate hourly fetcher. Surfaces signed grid import/export (`gridelectricityfrom`/`gridelectricityto` — solves the `grid_today` direction-ambiguity from Phase 3.4), eco metrics (`coal`, `treenum`), `*_yesterday` variants of all daily fields, and lifetime/monthly/annual rollups direct from cloud. Plus persistent lifetime battery in/out (RestoreSensor + coordinator-startup state seeding). Estimated ~5-10 min capture + ~3-4 hr integration. Surfaced session 22 from Bobsilvio comparison.
