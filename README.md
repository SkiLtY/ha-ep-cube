...
- [ ] **Phase 3.2** *(spike landed 2026-05-21 — implementation pending)* — Replace JSESSIONID-cookie auth with mobile-app Bearer token (`monitoring-eu.epcube.com/api/device/*`). Same host, simpler write API (one unified `switchMode` endpoint vs our three), programmatically-solvable captcha (no human in the loop), and a one-time email+password in config-flow replaces the hourly cookie paste. Full viability assessment at [docs/PHASE_3_2.md](docs/PHASE_3_2.md). One known risk to plan for: opening the official mobile app silently revokes write privileges on the HA token (mitigations: post-write read-back, auto re-auth, Predbat back-off). Worth landing before Phase 4 HACS so new users don't hit the JSESSIONID UX.
