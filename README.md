...
- [ ] **Phase 3.2** *(spike + capture + captcha-solver feasibility + write-revocation probes complete 2026-05-21 — implementation pending)* — Replace JSESSIONID-cookie auth with mobile-app Bearer token (`monitoring-eu.epcube.com/api/device/*`). Same host, unified write API (one `switchMode` endpoint vs our three — but requires full payload every call), programmatically-solvable captcha (no human in the loop), and a one-time email+password in config-flow replaces the hourly cookie paste. Live-cloud capture session validated end-to-end write path against our real device (read + write + read-back, no-op). Captcha-solver feasibility confirmed: pure-numpy port of `cv2.matchTemplate(TM_CCOEFF_NORMED)` is bit-exact with cv2 and hits the same 80% cube-acceptance rate over 20 live trials, so we avoid the ~30 MB `opencv-python-headless` dependency — only new dep will be `pycryptodome` (~1.8 MB) for the AES-ECB step. Confirmed corrections to the spike doc: `Authorization: Bearer <token>` requires the prefix, `Accept-Language: en-GB` should be used, `onlySave` is persistent state (must always send `"0"`), `weatherWatch` is a forecast-driven pre-charge feature that fights Predbat (must always send `"0"`). Write-revocation hypothesis from Bobsilvio's issue tracker (mobile-app login revokes HA token writes) probed across four scenarios — app open + passive view, app-initiated write to backup mode, 90 s sustained active app session, post-close recovery — and **rejected on every scenario**. The four planned mitigations downgrade accordingly: post-write read-back + auto re-auth on 403 still land (cheap and they handle normal token expiry anyway), Predbat-side back-off deferred. Full assessment + probe findings at [docs/PHASE_3_2.md](docs/PHASE_3_2.md), spike reference implementations at [spikes/captcha_spike.py](spikes/captcha_spike.py) + [spikes/revocation_probe_v2.py](spikes/revocation_probe_v2.py). Worth landing before Phase 4 HACS so new users don't hit the JSESSIONID UX.
