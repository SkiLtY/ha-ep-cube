...
| After a shim override reverts, the override slot stays on the cube's TOU schedule | The cube cloud silently ignores tier-list diffs on TOU→Self-Consumption transitions. The `switchMode` POST accepts the workStatus change but doesn't apply the bundled `midPeakTimeList` etc. Discovered 2026-05-21 live-cube verification — first `debug_freeze` left a £0.20 mid-peak slot persisted indefinitely. | `_strip_shim_slots` in `services.py` removes any tier-list slot whose price matches `SHIM_PRICE_OFF_PEAK` (£0.01), `SHIM_PRICE_MID_PEAK` (£0.20), or `SHIM_PRICE_PEAK` (£1.00) at baseline-snapshot time. Self-healing: the next override cleans up the previous run's debris. **User constraint**: don't manually configure cube TOU slots at £0.01, £0.20, or £1.00 — those prices are reserved as shim signatures and would be stripped on the next Predbat override. |
