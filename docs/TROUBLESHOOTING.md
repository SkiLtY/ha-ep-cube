...
| After a shim override reverts, the override slot stays on the cube's TOU schedule | The cube cloud silently ignores tier-list diffs on TOU→non-TOU transitions. The `switchMode` POST accepts the workStatus change but doesn't apply the bundled `midPeakTimeList` etc. Discovered 2026-05-21 live-cube verification — first `debug_freeze` left a £0.20 mid-peak slot persisted indefinitely. | Two layers: (1) `_strip_shim_slots` in `services.py` removes any tier-list slot whose price matches `SHIM_PRICE_OFF_PEAK` (£0.01), `SHIM_PRICE_MID_PEAK` (£0.20), or `SHIM_PRICE_PEAK` (£1.00) at baseline-snapshot time, so the next override doesn't compound. (2) `_revert_to_baseline` is a two-write sequence when baseline is non-TOU: write the cleaned schedule with `workStatus=2` first (cube applies the diff because no mode transition), then write `workStatus=baseline` to switch mode (schedule diff ignored on transition, but already clean). Adds one cloud write per non-TOU-baseline override cycle. **User constraint**: don't manually configure cube TOU slots at £0.01, £0.20, or £1.00 — those prices are reserved as shim signatures and would be stripped on the next Predbat override. |
